...
<div class="container">
    ...
    <div class="next">
        <?php
        $qry3 = mysqli_query($con, "SELECT * FROM lang");
        while ($n = mysqli_fetch_array($qry3)) {
            $langId = $n['lang_id'];
            ?>
            <form action="lang.php" method="GET">
                <button class="button button3" type="submit" name="lang_id" value="<?php echo $langId; ?>"><?php echo $n['lang']; ?></button>
            </form>
        <?php } ?>
    </div>
    ...
    if(!isset($_GET['lang_id'])){
        $langId = 1;
        $today = date("Y-m-d");
        $qry2 = mysqli_query($con, "SELECT * FROM tbl_movie WHERE lang_id = $langId");
